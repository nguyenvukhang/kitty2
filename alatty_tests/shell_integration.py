...
from alatty.constants import is_macos, kitten_exe, alatty_base_dir, shell_integration_dir, terminfo_dir
...
class ShellIntegration(BaseTest):
    ...
    def run_shell(self, shell='zsh', rc='', cmd='', setup_env=None):
        ...
        env['ALATTY_RUNNING_SHELL_INTEGRATION_TEST'] = '1'
        try:
            if self.with_kitten:
...
''') as pty:
        ...
        with self.run_shell(rc=f'''PS1="{ps1}"''') as pty:
            ...
            if not is_macos:  # Fails on older macOS like the one used to build alatty binary because of unicode encoding issues
                self.assert_command(pty)

...
is_ok_tmpdir(const char *x) {
    ...
    char path[2048];
    snprintf(path, sizeof(path), "%s/alatty-si-test-tmpdir-XXXXXXXXXXXX", x);
...
#define w(literal) write_to_membuf(&output, literal, sizeof(literal)-1)
    ...
    if (opts->wait_for_single_instance_window_close) {
        ...
        char addr[128];
        snprintf(addr, sizeof(addr), "alatty-os-window-close-notify-%d-%d", getpid(), geteuid());
...
single_instance_main(int argc, char *argv[], const CLIOptions *opts) {
    ...
    if (opts->instance_group) snprintf(addr_buf, sizeof(addr_buf), "alatty-ipc-%d-%s", geteuid(), opts->instance_group);
    else snprintf(addr_buf, sizeof(addr_buf), "alatty-ipc-%d", geteuid());

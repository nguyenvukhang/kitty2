...
is_boolean_flag(const char *x) {
    static const char *all_boolean_options = ALATTY_CLI_BOOL_OPTIONS;
    char buf[128];
...
handle_option_value:
    ...
    if (opts.version_requested) {
        if (isatty(STDOUT_FILENO)) {
            printf("\x1b[3malatty\x1b[23m \x1b[32m%s\x1b[39m created by \x1b[1;34mKovid Goyal\x1b[22;39m\n", ALATTY_VERSION);
        } else {
            printf("alatty %s created by Kovid Goyal\n", ALATTY_VERSION);
        }
        exit(0);
    }
    unsetenv("ALATTY_SI_DATA");

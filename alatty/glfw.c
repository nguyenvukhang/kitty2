...
create_os_window(PyObject UNUSED *self, PyObject *args, PyObject *kw) {
    ...
    if (is_first_window) {
        ...
        // See https://github.com/kovidgoyal/alatty/issues/7174#issuecomment-2000033873
    ...
    if (global_state.is_wayland) {
        ...
        if (temp_window == NULL) { fatal("Failed to create GLFW temp window! This usually happens because of old/broken OpenGL drivers. alatty requires working OpenGL %d.%d drivers.", OPENGL_REQUIRED_VERSION_MAJOR, OPENGL_REQUIRED_VERSION_MINOR); }

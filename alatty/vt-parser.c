...
static bool
parse_alatty_dcs(PS *self, uint8_t *buf, size_t bufsz) {
...
#define dispatch(prefix, func, delta) {\
    if (starts_with(prefix)) {\
        ...
        if (cmd) { \
            REPORT_OSC(func, cmd); \
            screen_handle_alatty_dcs(self->screen, #func, cmd); \
            Py_DECREF(cmd); \
        } else PyErr_Clear(); \
        return true; \
    }}
...
dispatch_dcs(PS *self, uint8_t *buf, size_t bufsz, bool is_extended UNUSED) {
    ...
    switch (buf[0]) {
        ...
        case '@':
            if (!parse_alatty_dcs(self, buf + 1, bufsz-1)) REPORT_UKNOWN_ESCAPE_CODE("DCS", buf);
            break;
        default:
            REPORT_UKNOWN_ESCAPE_CODE("DCS", buf);
            break;

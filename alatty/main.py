...
def macos_cmdline(argv_args: list[str]) -> list[str]:
    ...
    if '-1' in ans or '--single-instance' in ans:
        if 'ALATTY_SI_DATA' in os.environ:
            ...
            os.environ['ALATTY_LAUNCHED_BY_LAUNCH_SERVICES'] = '2'  # so that use_os_log is set in the re-execed process
            os.execl(alatty_exe(), 'alatty', *(ans + argv_args))
...
def _main() -> None:
    ...
    if cli_opts.single_instance:
        si_data = os.environ.pop('ALATTY_SI_DATA', '')
        if si_data:
            talk_fd = int(si_data)

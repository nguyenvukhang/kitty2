...
different easing functions for the two halves, for example: :code:`-1 linear ease-out`. alatty
...
:code:`2.0 ease-in linear`. alatty
supports all the :link:`CSS easing functions <https://developer.mozilla.org/en-US/docs/Web/CSS/easing-function>`.
'''
    )
...
opt('linux_bell_theme', '__custom', ctype='!bell_theme',
    long_text='''
The XDG Sound Theme alatty will use to play the bell sound.
...
Terminal applications can set this color using :ref:`The alatty color control <color_control>`
escape code.
...
Specify rules to filter out notifications sent by applications running in alatty.
...
alatty config directory (:file:`~/.config/alatty`). An annotated sample is
:link:`available <https://github.com/kovidgoyal/alatty/blob/master/docs/notifications.py>`.
''')
...
reloading the config is not supported. Note that if alatty is invoked with command line
...
of :code:`path` causes alatty to set it to a filesystem location containing the
alatty terminfo database. A value of :code:`direct` means put the entire database into
...
background color of the currently active alatty window and finally you can use an
...
Alternatively, you can just add :code:`map ctrl+l clear_terminal to_cursor_scroll active` to :file:`alatty.conf` which

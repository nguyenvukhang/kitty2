...
from alatty.launch import env_docs, remote_control_password_docs
from alatty.options.utils import env as parse_env
...
if TYPE_CHECKING:
    from alatty.cli_stub import RunRCOptions as CLIOptions
...
class Run(RemoteCommand):
    ...
    short_desc = 'Run a program on the computer in which alatty is running and get the output'
    desc = (
        'Run the specified program on the computer in which alatty is running. When STDIN is not a TTY it is forwarded'
...
type=bool-set
The executed program will have privileges to run remote control commands in alatty.
...
'''
    ...
    def response_from_alatty(self, boss: Boss, window: Optional[Window], payload_get: PayloadGetType) -> ResponseType:
        ...
        from alatty.launch import parse_remote_control_passwords

...
if TYPE_CHECKING:
    from alatty.cli_stub import LoadConfigRCOptions as CLIOptions
...
class LoadConfig(RemoteCommand):
    ...
    desc = (
        '(Re)load the specified alatty.conf config files(s). If no files are specified the previously specified config file is reloaded.'
        ' Note that the specified paths must exist and be readable by the alatty process on the computer that process is running on.'
        ' Relative paths are resolved with respect to the alatty config directory on the computer running alatty.'
    )
    options_spec = f'''\
--ignore-overrides
type=bool-set
By default, any config overrides previously specified at the alatty invocation command line

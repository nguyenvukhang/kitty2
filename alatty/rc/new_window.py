...
if TYPE_CHECKING:
    from alatty.cli_stub import NewWindowRCOptions as CLIOptions
...
class NewWindow(RemoteCommand):
    ...
    window_type/choices.alatty.os: One of :code:`alatty` or :code:`os`
...
the working directory for the alatty process you are talking to is.
...
default=alatty
choices=alatty,os
What kind of window to open. A alatty window or a top-level OS window.
...
'''
    ...
    def response_from_alatty(self, boss: Boss, window: Optional[Window], payload_get: PayloadGetType) -> ResponseType:
        ...
        return launch.response_from_alatty(boss, window, payload_get)
